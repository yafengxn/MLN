...
@interface MLNYYLabel ()

@property (nonatomic, strong) YYLabel *innerLabel;
...
//    CGSize size = [self.innerLabel sizeThatFits:CGSizeMake(maxWidth, maxHeight)];
    ...
    __block YYTextLayout *layout = nil;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (self.innerLabel.text || self.innerLabel.attributedText) {
            NSAttributedString *text = nil;
            if (self.innerLabel.attributedText) {
                text = self.innerLabel.attributedText;
            } else if (self.innerLabel.text) {
                text = [[NSAttributedString alloc] initWithString:self.innerLabel.text attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:self.fontSize]}];
            }
            layout = [YYTextLayout layoutWithContainerSize:CGSizeMake(maxWidth, maxHeight) text:text];
...
{
    if (!_innerLabel) {
        _innerLabel = [[YYLabel alloc] init];
        _innerLabel.font = kLuaDefaultFont;
        _innerLabel.displaysAsynchronously = YES;
    }
    return _innerLabel;
}
...
LUA_EXPORT_VIEW_BEGIN(MLNYYLabel)
LUA_EXPORT_VIEW_PROPERTY(text, "lua_setText:", "text", MLNYYLabel)
LUA_EXPORT_VIEW_PROPERTY(textAlign, "setTextAlignment:", "textAlignment", MLNYYLabel) // MLNTextAlign
LUA_EXPORT_VIEW_PROPERTY(fontSize, "lua_setFontOfSize:", "lua_fontSize", MLNYYLabel)
LUA_EXPORT_VIEW_PROPERTY(textColor, "setTextColor:", "textColor", MLNYYLabel)
LUA_EXPORT_VIEW_PROPERTY(lines, "setNumberOfLines:", "numberOfLines", MLNYYLabel)
LUA_EXPORT_VIEW_PROPERTY(breakMode, "setLineBreakMode:", "lineBreakMode", MLNYYLabel)
LUA_EXPORT_VIEW_PROPERTY(styleText, "setAttributedText:", "attributedText", MLNYYLabel)
LUA_EXPORT_VIEW_METHOD(padding, "lua_setPaddingWithTop:right:bottom:left:", MLNYYLabel)
LUA_EXPORT_VIEW_METHOD(setTextBold, "lua_setTextBold", MLNYYLabel)
LUA_EXPORT_VIEW_METHOD(setTextFontStyle, "lua_setTextFontStyle:", MLNYYLabel)
LUA_EXPORT_VIEW_METHOD(fontNameSize, "lua_fontName:size:", MLNYYLabel)
LUA_EXPORT_VIEW_METHOD(setAutoFit, "lua_setFonAutoFit:", MLNYYLabel)
LUA_EXPORT_VIEW_METHOD(setWrapContent, "setLua_wrapContent:",MLNYYLabel) //SDK>=1.0.2
LUA_EXPORT_VIEW_METHOD(setMaxWidth, "setLua_maxWidth:",MLNYYLabel) //SDK>=1.0.3，自适应时的限制
LUA_EXPORT_VIEW_METHOD(setMinWidth, "setLua_minWidth:",MLNYYLabel) //SDK>=1.0.3，自适应时的限制
LUA_EXPORT_VIEW_METHOD(setMaxHeight, "lua_setMaxHeight:",MLNYYLabel) //SDK>=1.0.3，自适应时的限制
LUA_EXPORT_VIEW_METHOD(setMinHeight, "lua_setMinHeight:",MLNYYLabel) //SDK>=1.0.3，自适应时的限制
LUA_EXPORT_VIEW_METHOD(setLineSpacing, "lua_setLineSpacing:",MLNYYLabel) //SDK>=1.0.3，自适应时的限制
LUA_EXPORT_VIEW_METHOD(a_setIncludeFontPadding, "lua_a_setIncludeFontPadding:", MLNYYLabel)
LUA_EXPORT_VIEW_END(MLNYYLabel, Label, YES, "MLNView", "initWithLuaCore:frame:")
